...
describe("mi turno", () => {
  ...
  it("el listado público no reparte los códigos ni los teléfonos", async () => {
    await reservar({ canchaId: canchaF7 });
    ...
    expect(datos[0].local.contacto).toBeUndefined();
    // Los nombres de los jugadores sí son públicos: es lo que mira
    // el que quiere anotarse de contra.
    expect(datos[0].local.jugadores).toHaveLength(3);
  });

  it("el que se anotó de contra sí ve el teléfono del local", async () => {
    const { datos: turno } = await reservar({ canchaId: canchaF7 });
    const { datos: contra } = await pedir(`/api/turnos/${turno.id}/contra`, {
      metodo: "POST",
      cuerpo: { equipo: equipo("Los Otros") },
    ...
    const { datos } = await pedir("/api/turnos/codigo/" + contra.codigo);
    expect(datos.local.contacto).toBe("3564 55-5555");
  });
});
